...
public class DMRC_MAE_CONTR_MENSUAL_gen {

    public DriverOracle driverOracle = new DriverOracle();
    //public String TABLE_NAME = "DMRC_MAE_FACTURA";

    public void init_DMRC_MAE_CONTR_MENSUAL(){

        try{
            System.out.println("Va a iniciar init_DMRC_MAE_CONTR_MENSUAL para generar datos ");
            Connection connection = driverOracle.getConnection();
            Statement stmt = connection.prepareStatement("insert into SYSTEM.DMRC_MAE_CONTR_MENSUAL(" +
                    "ID_NUM_CONTR, FCH_INFO, ID_COMPRA_VENTA_CARTERA , ID_SIT_CONTABLE_LOCAL , ID_TIPO_TASA , ID_SISTEMA_ORIGEN" +
                    ", FLG_AVAL_TECNICO  , VAL_PERIODO_AMORT_CAPITAL  , VAL_PERIODO_AMORT_INTERESES  , FCH_EXIG_CAPITAL" +
                    ", FCH_EXIG_INTERESES  , FCH_APERTURA_CONTR , FCH_FIN_CONTR  , IMP_CAPITAL_BANCO_NO_EXIG" +
                    ", IMP_CAPITAL_AJENO_NO_EXIG  , IMP_CAPITAL_BANCO_EXIG  , IMP_CAPITAL_AJENO_EXIG" +
                    ", IMP_LINEA_EXPRESS  , IMP_ACCESORIOS  , IMP_INTERESES_DEV_BANCO  , IMP_INTERESES_DEV_AJENO" +
                    ", IMP_INTERESES_EXIGIBLE_BANCO  , IMP_INTERESES_EXIGIBLE_AJENO  , IMP_RESERVA_INTERESES" +
                    ", IMP_RESERVA_INTERESES_CAPITAL  , IMP_DISPONIBLE  , IMP_RIESGO_FIRMA  , IMP_QUITA_CAPITAL" +
                    ", IMP_QUITA_INTERESES  , FCH_CARGA_INFO  , IMP_VENCIDO  , IMP_CASTIGADO  , ID_TIPO_PRODUCTO" +
                    ", ID_SUBTIPO_PRODUCTO , ID_DIVISA , ID_EMPRESA , ID_TIPO_AMORTIZACION , FCH_CASTIGO  , IMP_INICIAL_CONCEDIDO" +
                    ", FLG_CONTR_BAJO_LINEA_CREDITO  , FCH_INIC_CARENCIA_CAPITAL  , FCH_INIC_CARENCIA_INTERESES" +
                    ", FCH_FIN_CARENCIA_CAPITAL  , FCH_FIN_CARENCIA_INTERESES  , FCH_EXIG_CAPITAL_ULT_DIA_NAT" +
                    ", FCH_EXIG_INTERESES_ULT_DIA_NAT  , VAL_TIPO_INTERES , FLG_INTEGRACION_EXPEDIENTE  , ID_FINALIDAD_CONTRATO" +
                    ", IMP_INICIAL_PREVISTO_PAGOS  , IMP_VALOR_BIEN_FINANCIADO  , ID_ESTADO_MEXICO_JUICIO" +
                    ", TXT_DESC_SEG_RIESGOS , FLG_FIDEICOMISO_CONVENIO_JUD  , IMP_SUBCUENTA_VIVIENDA  , IMP_SEGURO_DESEMPLEO" +
                    ", PORC_SPREAD , FLG_CREDITO_GRUPAL  , FCH_PROXIMO_CORTE  , ID_TIPO_BLOQUEO , FLG_CUENTA_EJE" +
                    ", NUM_PAGOS_CONTRATO , IMP_PROXIMO_PAGO  , FCH_ULTIMO_PAGO  , FCH_LIQUIDACION , IMP_DACION_PAGO" +
                    ", IMP_QUEBRANTO_CASTIGO  , FCH_ENTRADA_ACLARACION  , FCH_PRIMER_INCUMPLIMIENTO  , IMP_SEGURO_VIDA" +
                    ", FCH_ULTIMA_COMPRA_DISPOSICION  , NUM_PAGOS_VENCIDOS , IMP_LIMITE  , ID_TIPO_FRAUDE , NUM_PAN" +
                    ", FLG_COBRANZA_DELEGADA  , TIPO_CAMBIO_MXN , PORC_COBERTURA_HIPOTECARIA , DESC_ESTADO_MEXICO" +
                    ", DESC_TERRITORIAL, DESC_CANAL_ADMISION  , DESC_SEG_RENTA  , FLG_CONTR_REPORTADO_NO_BURO, ID_SEG_CONTR_BDR ) " +
                    "values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?, ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?, " +
                    "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?, ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?, " +
                    "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?, ?,?,?,?,?,?,?,?,?)");
            ...
            for (int i = 8000000; i < 9000000; i++){
                ...
                ((PreparedStatement) stmt).setDate(2, java.sql.Date.valueOf("2018-10-15"));
                ...
                ((PreparedStatement) stmt).setString(6, String.valueOf(i));
                ...
                ((PreparedStatement) stmt).setString(37, String.valueOf(3));
                ((PreparedStatement) stmt).setDate(38, java.sql.Date.valueOf("2018-03-30"));
                ((PreparedStatement) stmt).setInt(39, 4);
                ...
                ((PreparedStatement) stmt).setDate(45, java.sql.Date.valueOf("2018-03-30"));
                ((PreparedStatement) stmt).setDate(46, java.sql.Date.valueOf("2018-03-30"));
                ((PreparedStatement) stmt).setInt(47, 4);
                ((PreparedStatement) stmt).setInt(48, 4);
                ((PreparedStatement) stmt).setString(49, String.valueOf(6));
                ...
                ((PreparedStatement) stmt).setString(60, "F");
                ...
                ((PreparedStatement) stmt).setString(74, "E");
                ...
                ((PreparedStatement) stmt).setString(82, String.valueOf(i));
